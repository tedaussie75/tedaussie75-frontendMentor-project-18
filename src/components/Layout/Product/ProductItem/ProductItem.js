...
function ProductItem({productToReset, productId, imgSrc, name, category,  price, cartItemState, cartMethods}) {
  ...
  return (
    <StyledProductItem className="productItem">
        <Image picture={true} pictureSrc={imgSrc} src={imgSrc.desktop} width="100%" outline={itemSelected && '3px solid var(--red)'}/>
        <Button cartBtn={true} text="Add to cart" transform="translateY(-50%);" selectableBtn={itemSelected} selectedElems={itemCount} eventClick={productMethods} children={<Image src={'./images/icon-add-to-cart.svg'}/>}/>
